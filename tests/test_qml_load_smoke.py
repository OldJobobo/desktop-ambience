...
class QmlLoadSmokeTests(unittest.TestCase):
    def test_all_ten_effect_files_load_with_complete_normalized_settings(self):
        qml = f'''
import Quickshell
import QtQuick
ShellRoot {{
  ...
  QtObject {{
    id: state
    property var activeEffects: ["auroraDrift", "cinematicLight", "crt", "dustMotes", "filmGrain", "godRays", "rainfall", "tacticalGrid", "trackingLines", "bokeh"]
  ...
  Timer {{
    ...
    onTriggered: {{
      ...
      if (!settingsLoader.item || !settingsLoader.item.hasLoaded || !stackLoader.item
          || stackLoader.item.activeProductionEffectCount !== 10) {{
        if (attempts > 30) {{
...
'''
        ...
        self.assertEqual(row["count"], 10, output[-2000:])
        self.assertEqual(
            row["ids"],
            ["auroraDrift", "cinematicLight", "crt", "dustMotes", "filmGrain", "godRays", "rainfall", "tacticalGrid", "trackingLines", "bokeh"],
        )
