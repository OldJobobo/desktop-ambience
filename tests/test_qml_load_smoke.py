...
class QmlLoadSmokeTests(unittest.TestCase):
    def test_all_nine_effect_files_load_with_complete_normalized_settings(self):
        qml = f'''
import Quickshell
import QtQuick
ShellRoot {{
  ...
  QtObject {{
    id: state
    property var activeEffects: ["auroraDrift", "cinematicLight", "crt", "dustMotes", "filmGrain", "godRays", "rainfall", "tacticalGrid", "trackingLines"]
  ...
  Timer {{
    ...
    onTriggered: {{
      ...
      if (!settingsLoader.item || !settingsLoader.item.hasLoaded || !stackLoader.item
          || stackLoader.item.activeProductionEffectCount !== 9) {{
        if (attempts > 30) {{
...
'''
        ...
        self.assertEqual(row["count"], 9, output[-2000:])
        self.assertEqual(
            row["ids"],
            ["auroraDrift", "cinematicLight", "crt", "dustMotes", "filmGrain", "godRays", "rainfall", "tacticalGrid", "trackingLines"],
        )
