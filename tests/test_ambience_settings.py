...
class AmbienceSettingsBehaviorTests(unittest.TestCase):
    def test_defaults_bounds_unknown_fields_and_order(self):
        ...
        self.assertEqual(set(defaults["effects"]), {"auroraDrift", "cinematicLight", "crt", "dustMotes", "drip", "filmGrain", "godRays", "rainfall", "tacticalGrid", "trackingLines", "bokeh", "nodeMesh"})
        self.assertEqual(defaults["effects"]["drip"]["direction"], "auto")
        self.assertEqual(defaults["effects"]["drip"]["intensity"], 1)
        self.assertEqual(defaults["effects"]["drip"]["accentBlend"], 0)
        self.assertFalse(defaults["effects"]["drip"]["bloodMode"])
        self.assertEqual(defaults["effects"]["drip"]["formationTime"], 3800)
