...
if ! command -v quickshell >/dev/null 2>&1 || [[ -z ${WAYLAND_DISPLAY:-} ]]; then
  echo "Phase 2 behavior checks require quickshell and an active Wayland session; refusing to report a partial pass" >&2
  exit 1
fi
...
if grep -Eq '[0-9]+ skipped' "$pytest_output"; then
  echo "Phase 2 behavior coverage was skipped" >&2
  exit 1
fi

echo "Phase 2 checks passed with runtime behavior coverage"
